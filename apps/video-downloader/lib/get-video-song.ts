...
export const APICheck = (item: Song, songId: string) => {
  ...
  const sunaAPIURL = `https://api.sunoapi.org/api/v1/mp4/record-info?taskId=${song.videoTaskId}`;
  return API.Get({
    url: sunaAPIURL,
    jwt: sunaAPI,
    jsonapi: false,
      ...
      if (response.data) {
        if (response.data.successFlag) {
          ...
          if (response.data.errorCode) {
            console.log('>>> Suno MP4 API Error:', response.data.errorCode);
            song.status = 'error';
          }
        }
      }
      if (response.code === 400) {
        song.status = 'MP4-deleted';
      }
...
const getVideoSong = ({ id, songId }: Parameters): Promise<Song> => {
    ...
    getSong({ id })
      .then((i: Song | null) => {
        ...
        const data = {
          ...
          author: 'TresHermanosMX',
          domainName: 'https://treshermanos.com.mx',
        };
        API.Post({
          url,
          data,
          ...
          .then((response: APIResponse) => {
            ...
            if (response.data && response.data.taskId) {
              song.videoTaskId = response.data.taskId;
            }
            updateSong(updateSongArray(item, song))
